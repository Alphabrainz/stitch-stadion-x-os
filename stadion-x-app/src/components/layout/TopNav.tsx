  ...
  return (
    <header className="fixed top-4 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-7xl z-50 bg-black/60 backdrop-blur-3xl border border-white/10 rounded-full flex justify-between items-center px-4 sm:px-6 h-16 shadow-[0_15px_40px_rgba(0,0,0,0.8)] transition-all duration-500">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        <button onClick={() => addToast('No new notifications', 'info')} className="hover:bg-white/10 transition-colors duration-300 p-2 sm:p-3 rounded-full active:scale-95 relative group text-white/80 mr-2">
          <span className="material-symbols-outlined text-[28px] group-hover:text-white transition-colors">notifications</span>
          {user?.role === 'employee' && (
        ...
        <div className="relative group cursor-pointer flex items-center gap-3 bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/10 p-1.5 pr-3 rounded-full transition-all duration-300">
          ...
          <span className="material-symbols-outlined text-[18px] text-white/40 group-hover:text-white/80 transition-colors hidden md:block">expand_more</span>
        </div>

        <button onClick={handleLogout} className="hover:bg-red-500/20 text-red-500 transition-colors duration-300 p-2 sm:p-3 rounded-full active:scale-95 group ml-1 sm:ml-2">
